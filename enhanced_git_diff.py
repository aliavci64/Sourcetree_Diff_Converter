...
class CrossPlatformManager:
    """Universal platform detection and configuration manager"""
    ...
    def _get_platform_config(self):
        ...
        configs = {
            'Windows': {
                'python_cmd': ['python', 'py', 'python.exe'],
                'downloads_path': self._get_windows_downloads(),
                'browser_cmd': 'startfile',
                'path_sep': '\\'
            },
            'Darwin': {  # macOS
    ...
    def _get_windows_downloads(self):
    ...
    def get_downloads_path(self):
        ...
        return self.config['downloads_path']
    
    def get_best_python_path(self):
        """Find the best Python executable for this platform"""
        import shutil
        
        # First, try current running Python
        current_python = sys.executable
        if self._validate_python_environment(current_python):
            return current_python
        
        # Try common Python commands
        for cmd in self.config['python_cmd']:
            python_path = shutil.which(cmd)
            if python_path and self._validate_python_environment(python_path):
                return python_path
        
        # Fallback: return current executable
        return current_python
    
    def _validate_python_environment(self, python_path):
        """Validate if Python environment has required modules"""
        try:
            # Quick validation test
            test_cmd = [python_path, '-c', 'import subprocess, pathlib, json; print("OK")']
...
class GitDiffAnalyzer:
    ...
    def get_git_diff(self, commit1, commit2=None, file_path=None):
        """Get git diff output between commits - with proper encoding handling"""
        ...
        try:
            # Windows için encoding handling
            result = subprocess.run(
                cmd, 
                capture_output=True, 
                text=True, 
                encoding='utf-8',
                errors='replace',  # Unicode hatalarını görmezden gel
                timeout=30
            )

            if result.returncode != 0:
                print(f"Git diff warning: {result.stderr}")

            return result.stdout if result.stdout else ""
        
        except subprocess.TimeoutExpired:
            print("Git diff timeout - repository too large")
            return ""
        except Exception as e:
    ...
    def get_changed_files(self, commit1, commit2=None):
        ...
        try:
            ...
            if result.returncode != 0:
                print(f"Git changed files warning: {result.stderr}")
                return []
            ...
            print("Git changed files timeout")
            return []
        except Exception as e:
    ...
    def get_commit_info(self, commit):
        """Get commit information with proper encoding"""
...
class HTMLReportGenerator:
    ...
    def _generate_html_content(self, diff_data, commit1_info, commit2_info=None, selected_files=None):
        ...
        title = "Git Diff Report"
        if commit2_info:
            subtitle = f"Comparing {commit1_info['hash']} → {commit2_info['hash']}"
        else:
            subtitle = f"Changes in {commit1_info['hash']}"
...
def ensure_chronological_order(commit1, commit2, analyzer):
    """Ensure commits are in chronological order (older first)"""
    if not commit2:
        return commit1, commit2
    
    try:
        info1 = analyzer.get_commit_info(commit1)
        info2 = analyzer.get_commit_info(commit2)
        
        if not info1 or not info2:
            print("⚠️  Could not get commit dates, keeping original order")
            return commit1, commit2
        
        try:
            cmd1 = ['git', '-C', str(analyzer.repo_path), 'show', '-s', '--format=%ct', commit1]
            cmd2 = ['git', '-C', str(analyzer.repo_path), 'show', '-s', '--format=%ct', commit2]
            ...
            if result1.returncode == 0 and result2.returncode == 0:
                ...
                if timestamp1 > timestamp2:
                    print(f"🔄 Reordering commits: {info2['hash']} (older) → {info1['hash']} (newer)")
                    return commit2, commit1
                else:
                    print(f"✅ Commits already in chronological order: {info1['hash']} (older) → {info2['hash']} (newer)")
                    return commit1, commit2
            
        except Exception as e:
            print(f"⚠️  Could not parse commit timestamps: {e}")
            
    except Exception as e:
        print(f"⚠️  Error checking commit order: {e}")
    
    print("📅 Keeping original commit order")
    return commit1, commit2
...
<body>
    <div class="container">
        <div class="header">
            ...
            <p>Choose which files to include in your comparison report</p>
        </div>
    ...
    downloads_path = platform_manager.get_downloads_path() / "selected_files.json"
...
def main():
    # Cross-platform manager'ı initialize et
    platform_manager = CrossPlatformManager()
    print(f"🖥️  Platform: {platform_manager.system}")
    ...
    try:
        ...
        if actual_commit2:  # İki commit varsa
            print("\n📅 Checking commit chronological order...")
            actual_commit1, actual_commit2 = ensure_chronological_order(
                actual_commit1, actual_commit2, analyzer
            )
        ...
        if not changed_files:
            print("❌ No files found with changes between the specified commits.")
        ...
        if platform_manager.open_in_browser(html_file):
